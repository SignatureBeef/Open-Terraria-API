...
namespace OTAPI.Modifications.Runtime
{
    ...
    class NpcMechSpawn
    {
        public NpcMechSpawn()
        {
            IL.Terraria.NPC.MechSpawn += il => Modify(il, (bool returnValue, float x, float y, int type, int num, int num2, int num3, int i, bool flag, Microsoft.Xna.Framework.Vector2 vector, float num6) =>
            {
                ...
                if (Hooks.NPC.MechSpawn == null || Hooks.NPC.MechSpawn(x, y, type, num, num2, num3, i, flag, vector, num6) == HookResult.Continue)
                {
                    return returnValue;
                }
                return false;
        ...
        protected void Modify(ILContext il, Func<bool, float, float, int, int, int, int, int, bool, Microsoft.Xna.Framework.Vector2, float, bool> callback)
        {
